...
def _get_scene_objects(basedir):
    ...
    for obj in object_pose[:,:2]:
        count += 1
        # print('obj:', obj, 'f:', f, 'c:', c, 'count:', count, 'max_obj:', max_obj)
...
def load_metis_data(basedir, selected_frames=None, use_obj=True, row_id=False):
    """loads vkitti data
    ...
    for camera in next(os.walk(rgb_dir))[1]:
        ...
        for frame in sorted(os.listdir(frame_dir)):
            if frame.endswith('.png'):
                frame_num = int(frame.split('rgb_')[1].split('.png')[0])

                if selected_frames[0] <= frame_num <= selected_frames[1]:
                    fname = os.path.join(frame_dir, frame)
                    imgs.append(imageio.imread(fname))

                    inst_frame = 'instancegt_' + frame.split('rgb_')[1].split('.png')[0] + '.png'
